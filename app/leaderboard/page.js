        ...
        <div className="flex-grow p-4">
          ...
          <div className="flex justify-center items-end gap-8 mb-6">
            ...
            <div className="flex flex-col items-center order-1 pt-4">
              ...
              <div className="w-16 h-16 rounded-full bg-gray-200 mb-2 flex items-center justify-center text-xl font-bold">
                {sortedData[1]?.name.charAt(0)}
              </div>
              <p className="font-semibold text-center">{sortedData[1]?.name}</p>
              <p className="text-sm text-gray-500 text-center">{sortedData[1]?.wins} wins</p>
              <p className="font-bold text-green-600 text-center">{formatAmount(sortedData[1]?.totalWinnings)}</p>
            </div>
  
            {/* 1st place */}
            <div className="flex flex-col items-center order-2 pt-0">
              <div className="text-4xl mb-2">🥇</div>
              <div className="w-20 h-20 rounded-full bg-gray-200 mb-2 flex items-center justify-center text-2xl font-bold">
                {sortedData[0]?.name.charAt(0)}
              </div>
              <p className="font-semibold text-center">{sortedData[0]?.name}</p>
              <p className="text-sm text-gray-500 text-center">{sortedData[0]?.wins} wins</p>
              <p className="font-bold text-green-600 text-center">{formatAmount(sortedData[0]?.totalWinnings)}</p>
            </div>
            ...
            <div className="flex flex-col items-center order-3 pt-8">
              ...
              <p className="font-semibold text-center">{sortedData[2]?.name}</p>
              <p className="text-sm text-gray-500 text-center">{sortedData[2]?.wins} wins</p>
              <p className="font-bold text-green-600 text-center">{formatAmount(sortedData[2]?.totalWinnings)}</p>
            </div>
          </div>
              ...
              {sortedData.slice(3).map((user, index) => (
                <div key={user.id} className="flex items-center p-3">
                  ...
                  <div className="text-right">
                    <p className="font-bold text-green-600">{formatAmount(user.totalWinnings)}</p>
                  </div>
                </div>
              ))}
            </div>
